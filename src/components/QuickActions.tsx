import { Ticket, MapPin, Shield, Zap, Leaf } from 'lucide-react';
...
const QuickActions: React.FC<QuickActionsProps> = ({ onAction }) => {
  ...
  const actions = [
    ...
    { icon: Shield, label: 'Safety Alert', color: 'bg-red-500 hover:bg-red-600', actionKey: 'safety_alert' },
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
          {actions.map((action, index) => (
            <button
              ...
              className={`p-6 rounded-lg ${action.color} text-white transition-all duration-200 transform hover:scale-105 hover:shadow-lg flex flex-col items-center space-y-3`}
            >
